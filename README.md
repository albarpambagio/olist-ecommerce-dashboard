# Olist Sales & Customer Analytics

## Executive Summary

**"3% Repeat Rate is Costing Olist R$558,000+ in Missed Revenue Annually"**

Olist processed 96,478 orders generating R$13.28M in revenue between 2016–2018. But the headline number masks a structural problem: **only 3% of customers ever place a second order**. RFM segmentation reveals that **24.1% of the customer base (23,272 customers) is already classified as At Risk** — customers who bought once but haven't returned.

The biggest lever for revenue growth is not acquiring new customers; it's converting the 81% of one-time buyers before they go cold.

> **Try the interactive dashboard:** `cd vizro_dashboard && pip install -r requirements.txt && python app.py` → Opens at http://localhost:8050

---

## Business Context

### What the Data Covers

| Metric | Value | Business Meaning |
|--------|-------|------------------|
| Total Revenue | R$13,279,837 | Baseline for growth calculations |
| Total Orders | 96,478 | Volume indicator |
| Average Order Value | R$119.81 | Typical transaction size |
| Unique Customers | ~93,000 | Actual distinct customers (vs 96,478 orders) |
| Repeat Customers | ~3,000 | Only ~3% return for a second order |

**Data Source:** Olist Brazilian E-Commerce (Kaggle), Sep 2016 – Oct 2018

### Where the Problem Lies

- **M1 Retention: ~0.5%** — Only 0.5% of customers return for a second purchase after their first month *(industry benchmark: 20%+)*¹
- **At Risk Segment: 23,272 customers (24.1%)** — Haven't purchased in 90+ days, showing early churn signals
- **Champions + Loyal: 35.9%** — The only segments generating repeat business

---

## Analysis

### RFM Segmentation

Customer segmentation using Recency, Frequency, and Monetary metrics reveals the retention problem:

| Segment | Customers | % of Base | Description |
|---------|-----------|-----------|-------------|
| **At Risk** | 23,272 | 24.1% | Haven't returned in 90+ days |
| **Loyal Customers** | 19,276 | 20.0% | Consistent repeat purchasers |
| **Recent Customers** | 15,528 | 16.1% | First purchase in last 30 days |
| **Champions** | 15,338 | 15.9% | High-value, frequent buyers |
| **Lost** | 15,320 | 15.9% | Haven't purchased in 180+ days |
| **Promising** | 7,744 | 8.0% | Show early engagement signals |

### Cohort Retention Pattern

Month-over-month retention analysis confirms the problem is systemic:

| Month Index | Avg Retention |
|-------------|---------------|
| M0 (first month) | 100% (baseline) |
| M1 (second month) | 0.5% |
| M3 (fourth month) | 0.3% |

**99.5% of customers never come back after their first purchase.**

---

## Findings

The data reveals a clear pattern: **Olist acquires customers effectively but fails to retain them.**

The At Risk segment represents the biggest opportunity:
- 23,272 customers who bought once and never returned
- Average first order value: R$119.81
- If just 20% (4,654 customers) return for a second order: **R$558,480 incremental revenue**

This isn't a product problem or a pricing problem — it's a retention problem. The same pattern holds across every monthly cohort, indicating a structural issue in the post-purchase experience.

### Revenue Opportunity Calculation

| Input | Value | Source |
|-------|-------|--------|
| At Risk customers | 23,272 | RFM analysis |
| Target conversion rate | 20% | Conservative assumption (e-commerce benchmark) |
| AOV | R$119.81 | Actual data |
| **Incremental revenue** | **R$558,480** | 23,272 × 20% × R$119.81 |

*Assumption: 20% conversion rate is based on industry benchmarks for retention campaigns (Klaviyo 2023). Actual results would depend on campaign quality and targeting.*

---

## Recommendations

### 1. Retention Campaign for At Risk Segment

**Target:** 23,272 At Risk customers

**Action:**
- Personalized "We miss you" email sequence
- 15% discount code for second purchase
- Product recommendations based on first purchase

**Expected Impact:** R$558,480 annually (20% conversion = 4,654 repeat customers)

---

## Technical Implementation

### Stack

- **PostgreSQL** (port 5433): Star schema data warehouse
- **Python**: ETL scripts (data loading, cleaning, transformation)
- **Vizro**: Interactive web dashboard (4 pages)
- **SQL**: Analytical views (RFM, cohort retention)

### Star Schema

```
fact_orders (central)
├── dim_customer (customer_id, customer_unique_id)
├── dim_product (product_id, category)
├── dim_seller (seller_id, state)
└── dim_date (order_date)
```

### Key Views

- `olist.customer_rfm`: RFM scores + segment classification
- `olist.cohort_retention`: Month-over-month retention by cohort

### Quick Start

```bash
# Launch Vizro dashboard
cd vizro_dashboard
uv sync
uv run python app.py
# Open http://localhost:8050
```

---

## Interview Talking Points

**"Tell me about a time you found a problem in your data."**
> "I discovered that the repeat customer rate was showing 0% initially — because the join was using `customer_id` instead of `customer_unique_id`. Each order has a unique customer_id even for repeat customers, so using it created a false result. After fixing the join to `customer_unique_id`, the real rate was 3%, which became the central finding of my entire analysis."

**"How did you decide which metrics to show?"**
> "I used a North Star approach: revenue is the primary metric because every team can pull some lever on it. For this project specifically, I prioritized repeat customer rate as the secondary metric because the data showed it was the biggest opportunity (3% vs 20%+ industry benchmark)."

**"Tell me about a time you quantified a business opportunity."**
> "I calculated that converting 20% of the 23,272 At Risk customers would generate R$558,480 in incremental revenue. The calculation uses actual AOV (R$119.81) from the data, with a 20% conversion assumption based on e-commerce industry benchmarks. Every input except the benchmark traces back to a SQL query."

---

## Source

**Dataset:** [Olist Brazilian E-Commerce (Kaggle)](https://www.kaggle.com/datasets/olistbr/brazilian-ecommerce)  
**License:** CC BY-NC-SA 4.0  
**Date Range:** September 2016 – October 2018

---

## Author

Albar Pambagio  
GitHub: [@albarpambagio](https://github.com/albarpambagio)

---

*¹ Industry benchmark: 20%+ repeat purchase rate is typical for healthy e-commerce businesses. Source: Klaviyo 2023 E-commerce Benchmark Report.*